...
bash "$CONTROL_ROOT/run_gpu_case_matrix.sh" > "$matrix_log" 2>&1 &
matrix_pid=$!
"$CONTROL_ROOT/run_slurm_job.sh" "$multinode_submit_log" \
    "$RESULT_ROOT/sai-cusolvermp-multinode-%j.out" \
    "$CONTROL_ROOT/test_gpu.sbatch" > "$multinode_log" 2>&1 &
multinode_pid=$!
